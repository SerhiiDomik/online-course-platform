...
class CourseViewsTests(TestCase):
    ...
    def test_course_detail_view(self):
        ...
        response = self.client.get(
            reverse("course:course-detail", kwargs={"course_pk": self.course.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_course_update_view(self):
        ...
        response = self.client.post(
            reverse("course:course-update", kwargs={"course_pk": self.course.pk}),
            data=form_data,
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_course_delete_view(self):
        self.client.login(username="TestUser", password="Password123_")
        response = self.client.post(
            reverse("course:course-delete", kwargs={"course_pk": self.course.pk})
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_only_creator_can_update_course(self):
        ...
        self.course.refresh_from_db()
        self.assertNotEqual(self.course.name, form_data["name"])
