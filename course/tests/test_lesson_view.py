...
class LessonViewsTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username="TestUser", password="Password123_")
        self.other_user = User.objects.create_user(username="OtherUser", password="Password123_")
        self.course = Course.objects.create(name="Test Course", description="Test Description", created_by=self.user)
        self.lesson = Lesson.objects.create(title="Test Lesson", content="Test Content", course=self.course)

    def test_lesson_detail_view(self):
        self.client.login(username="TestUser", password="Password123_")
        response = self.client.get(reverse('course:lesson-detail', kwargs={'course_pk': self.course.pk, 'lesson_pk': self.lesson.pk}))
    ...
    def test_lesson_create_view(self):
        ...
        response = self.client.post(reverse('course:lesson-create', kwargs={'course_pk': self.course.pk}), data=form_data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_lesson_update_view(self):
        ...
        response = self.client.post(reverse('course:lesson-update', kwargs={'course_pk': self.course.pk, 'lesson_pk': self.lesson.pk}), data=form_data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_lesson_delete_view(self):
        self.client.login(username="TestUser", password="Password123_")
        response = self.client.post(reverse('course:lesson-delete', kwargs={'course_pk': self.course.pk, 'lesson_pk': self.lesson.pk}))
        self.assertEqual(response.status_code, 302)
