...
class SavedCourseTests(TestCase):
    ...
    def setUp(self):
        ...
        self.course1 = Course.objects.create(
            name="Course 1", description="Description 1", created_by=self.user
        )
        self.course2 = Course.objects.create(
            name="Course 2", description="Description 2", created_by=self.user
        )

    def test_saved_course_list_view(self):
        SavedCourse.objects.create(
            user=self.user, course=self.course1, saved_at=timezone.now()
        )
    ...
    def test_save_course_view(self):
        response = self.client.post(
            reverse("course:save-course", kwargs={"course_pk": self.course2.pk})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            SavedCourse.objects.filter(user=self.user, course=self.course2).exists()
        )
    ...
    def test_remove_course_view(self):
        ...
        response = self.client.post(
            reverse("course:remove-saved-course", kwargs={"course_pk": self.course1.pk})
        )
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            SavedCourse.objects.filter(user=self.user, course=self.course1).exists()
        )
